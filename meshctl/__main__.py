...
def _do_exchange(port: str, op: str, params: dict | None, timeout: float) -> dict | int:
    """One owned exchange; returns the reply dict or an exit code."""
    try:
        with serial_link.SerialSession(port) as session:
            reply, events = session.exchange(op, params, timeout)
    except serial_link.PortBusyError:
        return _err(f"PORT_BUSY: {port} is owned by another process (one owner per port)")
    except serial_link.TimeoutError as exc:
        return _err(f"timeout: {exc}")
    except ValueError as exc:
        return _err(str(exc))
    except RuntimeError as exc:
        return _err(str(exc))
    except OSError as exc:
        return _err(f"serial error: {exc}")
    for evt in events:
        if isinstance(evt, dict) and evt.get("event") == "received":
            _note_received(port, evt)
    if not reply.get("ok"):
...
def cmd_status(args: argparse.Namespace) -> int:
    reply = _do_exchange(args.port, "status", None, args.timeout)
...
def cmd_provision(args: argparse.Namespace) -> int:
    ...
    reply = _do_exchange(args.port, "provision", {"label": args.label}, args.timeout)
    if isinstance(reply, int):
        return reply
    _print_result(reply)
    return 0
...
def cmd_time(args: argparse.Namespace) -> int:
    if args.time_cmd == "status":
        reply = _do_exchange(args.port, "time_status", None, args.timeout)
        if isinstance(reply, int):
            return reply
        _print_result(reply)
        return 0
    ...
    reply = _do_exchange(args.port, "time_set", {"unix_seconds": secs}, args.timeout)
    if isinstance(reply, int):
        return reply
    _print_result(reply)
    return 0
...
def cmd_radio(args: argparse.Namespace) -> int:
    if args.radio_cmd == "on":
        reply = _do_exchange(args.port, "radio_set", {"enabled": True}, args.timeout)
    elif args.radio_cmd == "off":
        reply = _do_exchange(args.port, "radio_set", {"enabled": False}, args.timeout)
    else:
        reply = _do_exchange(args.port, "radio_arm_next_boot", None, args.timeout)
    if isinstance(reply, int):
        return reply
    _print_result(reply)
    return 0
...
def cmd_ping(args: argparse.Namespace) -> int:
    ...
    reply = _do_exchange(args.port, "ping", {"count": args.count}, args.timeout)
    if isinstance(reply, int):
        return reply
    _print_result(reply)
    return 0
...
def cmd_send(args: argparse.Namespace) -> int:
    ...
    try:
        with serial_link.SerialSession(args.port) as session:
            reply, events = session.exchange("send", params, args.timeout)
    except serial_link.PortBusyError:
        return _err(f"PORT_BUSY: {args.port} is owned by another process (one owner per port)")
    except serial_link.TimeoutError as exc:
...
def cmd_contacts(args: argparse.Namespace) -> int:
    reply = _do_exchange(args.port, "contacts", None, args.timeout)
    if isinstance(reply, int):
        return reply
...
def cmd_delete(args: argparse.Namespace) -> int:
    ...
    if cid is None:
        return _err(describe)
    reply = _do_exchange(args.port, "contact_delete", {"contact_id": cid}, args.timeout)
    if isinstance(reply, int):
        return reply
...
def cmd_block(args: argparse.Namespace, blocked: bool) -> int:
    ...
    reply = _do_exchange(args.port, op, {"contact_id": cid}, args.timeout)
    if isinstance(reply, int):
        return reply
...
def cmd_pair_export(args: argparse.Namespace) -> int:
    ...
    reply = _do_exchange(args.port, op, None, args.timeout)
    if isinstance(reply, int):
        return reply
...
def cmd_pair_import(args: argparse.Namespace) -> int:
    ...
    reply = _do_exchange(args.port, "pair_import", params, args.timeout)
    if isinstance(reply, int):
        return reply
    result = reply.get("result", {})
...
def cmd_listen(args: argparse.Namespace) -> int:
    try:
        session = serial_link.SerialSession(args.port).open()
    except serial_link.PortBusyError:
        return _err(f"PORT_BUSY: {args.port} is owned by another process (one owner per port)")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--port", default=None, help="serial path (required except for ports)")
    parser.add_argument("--timeout", type=float, default=serial_link.DEFAULT_TIMEOUT, help="reply deadline in seconds")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd in ("ports", "boards", "tui", "flash", "elf", "uf2", "reboot", "records"):
        ...
        print("meshctl: error: --port is required (except for ports, boards, tui, flash, elf, uf2, reboot, records)", file=sys.stderr)
        return 2
    if args.cmd == "status":
